...
describe('SharingService.setOwnSharing', () => {
  it('pause de participant ativo → paused + consent_log', async () => {
    ...
    const out = await new SharingService(db, new NoopRealtime(), noopNotif).setOwnSharing(P, 'g1', 'pause');
  ...
  it('supervised_participant não pode pausar (403)', async () => {
    ...
    const err = await new SharingService(db, new NoopRealtime(), noopNotif).setOwnSharing(P, 'g1', 'pause').catch((e) => e);
    expect((err as HttpException).getStatus()).toBe(403);
  });
  it('resume a partir de active é inválido (409)', async () => {
    const { db } = mkDb({ role: 'participant', sharing_status: 'active' });
    const err = await new SharingService(db, new NoopRealtime(), noopNotif).setOwnSharing(P, 'g1', 'resume').catch((e) => e);
