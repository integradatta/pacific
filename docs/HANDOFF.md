...
- **Validado:** lint 4/4 ✓ · test **api 72 + shared 32** ✓ · web build ✓ · RLS 6/6 ✓ · magic-link 7/7 ✓ · **smoke test logado em prod** (criar → pagar parcial → quitar → dashboard reflete) ✓.
- **Hardening:** `POST /debts/:id/payments` ignora `amount` negativo (clamp ≥ 0).
- **Polish:** histórico passou a registrar **"Operação quitada"** (derivado de `settledAt`, kind `paid`, glifo ✓ verde).
- **Validação VISUAL em PROD (Puppeteer dirigindo Chrome contra a app no Vercel):** confirmados — Carteira vazia c/ CTA "Nova operação", Notificações vazias c/ CTA "Gerar alertas", detalhe da operação (breakdown juros + caixa de pagamento + tags + histórico), pós-pagamento parcial ("Devido agora"), estado **Quitada**, e Dashboard (A receber/Recebido/Quitadas + Horizonte sonar). **Importante p/ CORS:** a API só aceita o `WEB_ORIGIN` do Vercel — testes de browser autenticado devem rodar contra `pacific-web-chi.vercel.app`, não `localhost`.
- 🧹 **Dados de teste a apagar:** ~5 tenants `VALIDACAO-TEMP (apagar)` + usuários Supabase `validacao-*@pacific-validation.dev` (criados nas validações; isolados da carteira real). Limpeza: `DELETE` keyed pelo nome do tenant (filhos primeiro) + apagar os usuários no Supabase Auth.
